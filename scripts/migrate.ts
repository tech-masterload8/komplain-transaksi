...
async function connect(opts: {
  ...
  try {
    ...
    console.log(`Terhubung: ${opts.user}@${opts.host}:${opts.port}/${opts.database}`);
    return client;
  } catch (error) {
    ...
    if (code === "28P01") {
      throw new Error(
        `Password ditolak untuk user "${opts.user}" ke database "${opts.database}". Salin password dari aaPanel → PgSQL, lalu Recreate container.`,
      );
    }
    if (code === "28000" || /no pg_hba.conf entry/i.test(message)) {
      throw new Error(
        `PostgreSQL menolak koneksi Docker (${message}). ` +
          `Di pg_hba.conf tambahkan: host all all 172.16.0.0/12 scram-sha-256  lalu reload PostgreSQL. ` +
          `Migrasi hanya ke database "${opts.database}" sebagai user "${opts.user}", bukan ke otomaxbank.`,
      );
    }
    if (code === "3D000" || /database .* does not exist/i.test(message)) {
      throw new Error(
        `Database "${opts.database}" belum ada. Buat di aaPanel → PgSQL (user "${opts.user}"), tanpa memakai otomaxbank.`,
      );
    }
    throw error;
  }
...
async function main() {
  ...
  const appUser = env("APP_DB_USER", "komplain");
  ...
  console.log(`Migrasi skema aplikasi ke ${appUser}@${appDatabase} (otomaxbank tidak disentuh)`);
